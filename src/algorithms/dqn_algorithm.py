...
Implements standard Q-learning for offline RL.
"""
...
class DQNDataset(IterableDataset):
    """Iterable dataset for DQN training with DataLoader."""
...
class DQNAlgorithm(BaseRLAlgorithm):
    ...
    Extends BaseRLAlgorithm with DQN-specific functionality.
    """
    ...
    def build_network(self) -> None:
        ...
        if torch.cuda.device_count() > 1:
            print(f"Using {torch.cuda.device_count()} GPUs for DQN")
        ...
        print("DQN networks built successfully")
    ...
    def act(self, state_options: List[Tuple], epsilon: float = 0.0) -> Optional[int]:
        ...
        if random.random() > current_epsilon:
            ...
            with torch.no_grad():
                ...
                q_values = self.q_network(id_features_tensor, numeric_features_tensor)
    ...
    def learn(self, experiences: List[Experience]) -> float:
        if not experiences:
            return 0.0
        batch_data = self._dqn_collate_fn([experiences])
        for key in batch_data:
            if isinstance(batch_data[key], torch.Tensor):
                batch_data[key] = batch_data[key].to(self.device)
        # DQN target calculation
        q_values = self.q_network(batch_data["id_features"], batch_data["numeric_features"])
        with torch.no_grad():
            next_q = self.target_network(batch_data["next_id_features"], batch_data["next_numeric_features"])
            max_next_q = next_q.max(1, keepdim=True)[0]
        q_targets = batch_data["rewards"] + (self.gamma * max_next_q * (1 - batch_data["dones"]))
    ...
    def train(self, train_events: List[Dict], val_events: List[Dict]) -> Dict[str, Any]:
        if not self.q_network:
            self.build_network()
        print(f"Starting DQN training with {len(train_events)} train events, {len(val_events)} val events")
    ...
    def _dqn_collate_fn(self, experiences_list: List[List[Experience]]) -> Dict[str, torch.Tensor]:
